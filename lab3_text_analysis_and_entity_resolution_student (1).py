...
def simpleTokenize(string):
    ...
    if len(string)>0:
        ...
        string = string.strip()
        result = re.split(split_regex,string)
    else:
        result =[]
    return result
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda (record,item): len(item)).reduce(lambda a,b: a+b)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    sortedRDD = vendorRDD.sortBy(lambda x: -len(x[1]))
    return sortedRDD.take(1)
...
def tf(tokens):
    ...
    for elem in dic.keys():
        dic[elem] = float(dic[elem]/len(tokens)) 
    return dic
...
def idfs(corpus):
    ...
    tokenCountPairTuple = uniqueTokens.map(lambda x:(x,1.0)).reduceByKey(lambda a,b:a+b)
    tokenSumPairTuple = tokenCountPairTuple.map(lambda (token,total):(token,N/total))
...
avgSimDups = trueDupsRDD.map(lambda (a,(b,c)): b).mean()
nonDupsRDD = (sims
              .leftOuterJoin(goldStandard)
              .filter(lambda (k, v): v[1]==None))
avgSimNon = nonDupsRDD.map(lambda (a,(b,c)): b).mean()
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda (rec,tokens):(rec,tfidf(tokens, idfsFullBroadcast.value)))
googleWeightsRDD = googleFullRecToToken.map(lambda (rec,tokens):(rec,tfidf(tokens, idfsFullBroadcast.value)))
...
'''
pairs=[]
for (k,v) in record[0][1].items():
        pairs.append((k,record[0][0]))
print pairs
'''
def invert(record):
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    for token in tokens:
        amazonVec[token] = amazonWeightsBroadcast.value[amazonRec][token]
        googleVec[token] = googleWeightsBroadcast.value[googleRec][token]
    s = dotprod(amazonVec,googleVec)
    value = s/(amazonNormsBroadcast.value.get(amazonRec)*googleNormsBroadcast.value.get(googleRec))
