...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    for elem in ratings:
        sum+=elem
    return (ID,(len(ratings),sum/len(ratings)))
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(lambda line:getCountsAndAverages(line))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                       ...
                       .map(lambda ((a,b),(c,d)):math.pow(c-d,2)))
...
myRatedMovies = [
     (myUserID,318 ,3),(myUserID,1387,5),(myUserID,1214,3),
    (myUserID,593,3),(myUserID,1219,2),(myUserID,1,5),
    (myUserID,1240,5),(myUserID,2762,1),(myUserID,2571,4)
    ,(myUserID,2804,5)
...
myList = []
for elem in myRatedMovies:
    myList.append(elem[1])

myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda (x,y): x not in myList))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (movieID, (ratingscount, avg)):(movieID, ratingscount))
'''
